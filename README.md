...
When I trigger an event (like a click) on a child element, that event automatically  triggering the same event on its parent and all upper ancestors one by one.

### 4. What is Event Delegation in JavaScript? Why is it useful?
Instead of putting a click listener on every child ,we set only one listener in a parent .When a child is clicked, the click moves up to the parent by Event Bubbling. The parent catch it and uses (event.target) to see exactly which child was clicked.
